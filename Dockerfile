...
RUN add-apt-repository 'deb [arch=amd64,i386,ppc64el] http://mariadb.kisiek.net/repo/10.1/ubuntu trusty main'

RUN echo "mariadb-server-10.1 mysql-server/root_password password ''" | debconf-set-selections
RUN echo "mariadb-server-10.1 mysql-server/root_password_again password ''" | debconf-set-selections
